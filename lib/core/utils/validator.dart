...
//---------------------------------------------------------phone number
  static String? phoneNumberValidation(String? number) {
    final RegExp egyptianNumberRegex = RegExp(
      r'^(\+201|01|00201)[0-2,5]{1}[0-9]{8}$',
    );
    final RegExp internationalNumberRegex = RegExp(r'^\+[0-9]{7,15}$');

    if (number == null || number.trim().isEmpty || number == '+2') {
      return LocaleKeys.validation_phoneEmpty.tr();
    } else if (!egyptianNumberRegex.hasMatch(number) &&
        !internationalNumberRegex.hasMatch(number)) {
